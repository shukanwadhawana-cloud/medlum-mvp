...
function PrintInner() {
  ...
  if (err) {
    return (
      <div className="p-6">
        ...
        <Link href="/billing" className="text-[#c2183a] text-sm">← Billing</Link>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-lg bg-white p-6 text-[#140a1f] print:p-0">
      <div className="mb-4 flex items-start justify-between gap-3 print:hidden">
        <Link href="/billing" className="text-sm text-[#c2183a]">← Billing</Link>
        <button type="button" onClick={() => window.print()} className="rounded-lg bg-[#c2183a] px-3 py-2 text-sm font-semibold text-white">Print</button>
      </div>
      ...
      <div className="mt-3 flex justify-between text-sm">
        ...
        <div className="text-right text-xs text-gray-500">
          <p>{inv.createdAt ? new Date(inv.createdAt).toLocaleString() : ""}</p>
        ...
        <tbody>
          {(data.lines || []).map((l: any, i: number) => (
            <tr key={i} className="border-b border-gray-100">
              <td className="py-1.5">{l.description}{l.category && <span className="block text-[10px] text-gray-400">{l.category}</span>}</td>
      ...
      <div className="mt-3 space-y-1 text-sm">
        ...
        <div className="flex justify-between text-gray-600"><span>Paid</span><span>{money(inv.paid)}</span></div>
        <div className="flex justify-between"><span>Balance</span><b>{money(inv.balance)}</b></div>
      </div>
      {(data.payments || []).length > 0 && (
        <div className="mt-4 border-t pt-2">
          <p className="text-xs font-semibold uppercase text-gray-500">Payments</p>
          {(data.payments || []).map((p: any) => (
            <p key={p.id} className="text-xs text-gray-600">{money(p.amount)} · {p.method}{p.paidAt ? ` · ${new Date(p.paidAt).toLocaleString()}` : ""}</p>
          ))}
        </div>
      )}
