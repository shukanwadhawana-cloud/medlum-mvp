...
function MoreSidebar({ open, onClose, pathname, onLogout, isOwner }: { open: boolean; onClose: () => void; pathname: string; onLogout: () => void; isOwner: boolean }) {
  ...
  const groups = moreItems.reduce((acc, item) => { (acc[item.group] ||= []).push(item); return acc; }, {} as Record<string, typeof moreItems>);
  return createPortal(
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {Object.entries(groups).map(([group, items]) => (
            <div key={group} className="mb-4">
              <p className="mb-1 px-2 text-[10px] font-semibold uppercase tracking-wide text-gray-400">{group}</p>
              {items.map((item) => (
          ...
          <Link href="/dashboard" onClick={onClose} className={`mb-0.5 flex items-center gap-2 rounded-lg px-2 py-2.5 text-sm ${pathname === "/dashboard" ? "bg-red-50 font-medium text-[#c2183a]" : "text-[#140a1f]"}`}><Icon name="clinic" size={16} /><span>Dashboard</span></Link>
        </div>
