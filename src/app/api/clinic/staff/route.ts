...
export async function POST(req: Request) {
  ...
  await writeAudit({
    ...
    entity: "ClinicMember",
    entityId: member.id,
    meta: { clinicId: ctx.membership.clinicId, staffDoctorId: doctor.id, role },
  });
...
export async function PATCH(req: Request) {
  ...
  if (action === "role") {
    ...
    await writeAudit({ doctorId: ctx.session.doctorId, action: "CLINIC_MEMBER_ROLE_CHANGED", entity: "ClinicMember", entityId: id, meta: { clinicId: ctx.membership.clinicId, targetDoctorId: target.doctorId, previousRole, newRole: role } });
    return NextResponse.json({ success: true, member });
  }
  ...
  await writeAudit({ doctorId: ctx.session.doctorId, action: isActive ? "CLINIC_MEMBER_REACTIVATED" : "CLINIC_MEMBER_DEACTIVATED", entity: "ClinicMember", entityId: id, meta: { clinicId: ctx.membership.clinicId, targetDoctorId: target.doctorId } });
  return NextResponse.json({ success: true, member });
}
