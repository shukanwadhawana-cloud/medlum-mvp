...
export default function TelemedicineVideoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell>
    <div className="mb-4 flex items-start justify-between gap-3">
      <div><Link href="/dashboard" className="text-xs text-[#c2183a]">← Dashboard</Link><h1 className="mt-1 text-xl font-bold">Video consultation</h1><p className="text-sm text-gray-500">Waiting room → video → consultation → completion.</p></div>
      <span className="rounded-full bg-gray-100 px-3 py-1 text-xs">{session?.status}</span>
    </div>
    {error && <div className="mb-3 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div>}

    <section className="rounded-2xl border bg-white p-4 mb-3">
      <div className="text-sm font-semibold">Consultation room</div>
      <div className="mt-1 text-xs text-gray-500">Provider: {session?.provider || "external"} · Session: {session?.id}</div>
      <div className="mt-3 flex flex-wrap gap-2">
        {session?.status === "Scheduled" && <button disabled={busy} onClick={() => update("Waiting")} className="rounded-xl border px-4 py-2.5 text-sm font-medium">Open waiting room</button>}
        {session?.status === "Waiting" && <button disabled={busy} onClick={() => update("Active")} className="rounded-xl bg-[#140a1f] px-4 py-2.5 text-sm font-medium text-white">Start consultation</button>}
        {active && <button disabled={busy} onClick={() => update("Completed")} className="rounded-xl border px-4 py-2.5 text-sm font-medium">Complete consultation</button>}
        {canJoin && session?.meetingUrl && <a href={session.meetingUrl} target="_blank" rel="noreferrer" className="rounded-xl border px-4 py-2.5 text-sm font-medium">Open video in new tab</a>}
        {canJoin && session?.meetingUrl && <a href={`/telemedicine/${encodeURIComponent(session.id)}`} className="rounded-xl border px-4 py-2.5 text-sm">Refresh room</a>}
      </div>
    </section>

    {canJoin && session?.meetingUrl ? <section className="overflow-hidden rounded-2xl border bg-black">
      <div className="aspect-video w-full"><iframe title="MedLum video consultation" src={session.meetingUrl} allow="camera; microphone; fullscreen; display-capture; autoplay" className="h-full w-full border-0" /></div>
