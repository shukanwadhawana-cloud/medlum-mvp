...
const Icon = ({ name, size = 16 }: { name: string; size?: number }) => {
  ...
  const paths: Record<string, React.ReactNode> = {
    ...
    help: (<><circle cx="12" cy="12" r="9" /><path d="M9.5 9.5a2.5 2.5 0 1 1 3.5 2.3c-.7.4-1.5 1-1.5 2.2" /><path d="M12 17h.01" /></>),
...
function MoreSidebar({
  ...
  const ui = (
    <div className="fixed inset-0" style={{ zIndex: 9999 }} role="dialog" aria-modal="true" aria-label="More menu">
      ...
      <aside className="absolute inset-y-0 right-0 flex w-[min(100%,20rem)] flex-col bg-white shadow-2xl" style={{ zIndex: 10000 }}>
        <div className="flex items-center justify-between border-b px-4 py-3">
          ...
          <button type="button" onClick={onClose} className="grid h-10 w-10 place-items-center rounded-lg border text-[#140a1f]" aria-label="Close">
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-3">
          ...
          <div className="mt-2 border-t pt-2">
            <Link href="/more" onClick={onClose} className="flex min-h-12 items-center gap-3 rounded-xl px-3 text-sm text-gray-700 active:bg-gray-50">
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const moreActive = moreNav.some((i) => isActive(pathname, i.href)) || pathname.startsWith("/more");
      ...
      <header className="sticky top-0 z-40 bg-[#140a1f] text-white shadow-sm">
        <div className="mx-auto w-full max-w-7xl px-3 sm:px-4 lg:px-6">
          <div className="flex h-12 items-center gap-2 sm:h-14 sm:gap-3">
            <Link href="/dashboard" className="flex shrink-0 items-center gap-2 text-base font-bold sm:text-lg" aria-label="MedLum dashboard">
            ...
            <nav aria-label="Mobile navigation" className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto overscroll-x-contain scrollbar-none md:hidden">
              {primaryNav.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
