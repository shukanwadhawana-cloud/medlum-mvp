...
function makePdf(lines: string[]) {
  ...
  const content = [
    ...
    "/F1 11 Tf",
    "50 790 Td",
    ...safe.flatMap((line) => [`(${esc(line)}) Tj`, "0 -17 Td"]),
    "ET",
  ].join("\n");
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const catalog: Record<string, { name: string; amount: number }> = {
      professional: {
        name: "Professional",
        amount: interval === "yearly" ? 19990 : 1999,
      },
      clinic_plus: {
        name: "Clinic Plus",
        amount: interval === "yearly" ? 49990 : 4999,
      },
    };
    ...
    if (!plan) {
      return NextResponse.json(
        { success: false, error: "Paid plan required" },
        { status: 400 },
      );
    }
    ...
    if (!membership) {
      return NextResponse.json(
        {
          success: false,
          error: "No active clinic is associated with this account",
        },
        { status: 400 },
      );
    }

    const invoiceNumber = `MEDLUM-SUB-${new Date()
      .toISOString()
      .slice(0, 10)
      .replaceAll("-", "")}-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
    const now = new Date();
    const amount = plan.amount.toLocaleString("en-IN");
    ...
    await writeAudit({
      ...
      action: "create",
      entity: "SubscriptionProForma",
      entityId: membership.clinic.id,
      meta: {
        invoiceNumber,
        plan: plan.name,
        planId,
    ...
    return new NextResponse(pdf, {
      ...
      headers: {
        ...
        "X-MedLum-Razorpay-Key-Configured": process.env.RAZORPAY_KEY_ID
          ? "true"
          : "false",
      },
    });
    ...
    return NextResponse.json(
      { success: false, error: "Unable to generate subscription pro forma" },
      { status: 500 },
    );
  }
}
