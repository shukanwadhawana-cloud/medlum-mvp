...
export default function ClinicPage() {
  ...
  useEffect(() => {
    if (data?.currentMember && ["Owner", "Admin"].includes(data.currentMember.role)) {
      void loadEkaStatus();
    }
  }, [data?.currentMember]);
  ...
  async function onboardEka(e: React.FormEvent) {
    ...
    try {
      const r = await fetch("/api/interoperability/eka/onboard", {
        ...
        body: JSON.stringify({
          clinicId: ekaStatus.clinicId,
          hipId: ekaHipId.trim(),
          name: ekaName.trim() || data?.clinic?.name || "MedLum Clinic",
        }),
      });
      const j = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(j.error || "EKA facility onboarding failed.");
      setMessage("EKA ABDM facility onboarding request completed successfully.");
      setEkaHipId("");
      setEkaName("");
    } catch (e) {
      setError(e instanceof Error ? e.message : "EKA facility onboarding failed.");
    } finally {
      setEkaLoading(false);
    }
  }
  ...
  async function memberAction(method: string, id: string, memberRole?: string) {
    const r = await fetch("/api/clinic", { method, credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(method === "PATCH" ? { id, role: memberRole } : { id }) });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) setError(j.error || "Action failed."); else await load();
  }
  ...
  return <AppShell>
    <div className="mb-4">
      ...
      <h1 className="mt-1 text-xl font-bold">Clinic</h1>
      <p className="text-sm text-gray-500">Shared workspace, consultant access and patient portal controls.</p>
    </div>
    ...
    <section className="rounded-2xl bg-white p-4 border mb-3">
      <div className="text-lg font-semibold">{data?.clinic?.name}</div>
      <div className="text-xs text-gray-500">Your role: {data?.currentMember?.role} · {data?.members?.length || 0} members</div>
    </section>

    {canManage && <section className="rounded-2xl bg-white p-4 border mb-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="font-semibold">ABDM / EKA integration</h2>
          <p className="text-xs text-gray-500 mt-1">Connect this clinic to EKA's ABDM facility onboarding flow.</p>
        </div>
        <span className={`rounded-full px-2.5 py-1 text-xs ${ekaStatus?.configured ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}>
          {ekaStatus?.configured ? "Configured" : "Not configured"}
        </span>
      </div>
      {ekaStatus?.configured ? <form onSubmit={onboardEka} className="mt-3 space-y-2">
        <input required value={ekaHipId} onChange={e => setEkaHipId(e.target.value)} placeholder="EKA HIP ID" className="w-full rounded-xl border px-3 py-2.5 text-sm" />
        <input value={ekaName} onChange={e => setEkaName(e.target.value)} placeholder={`Facility name (default: ${data?.clinic?.name || "clinic"})`} className="w-full rounded-xl border px-3 py-2.5 text-sm" />
        <button disabled={ekaLoading || !ekaHipId.trim()} className="w-full rounded-xl bg-[#140a1f] px-4 py-2.5 text-sm font-medium text-white disabled:opacity-50">
          {ekaLoading ? "Connecting…" : "Onboard facility with EKA"}
        </button>
      </form> : <p className="mt-3 rounded-xl bg-amber-50 p-3 text-xs text-amber-800">EKA credentials are not configured on this deployment. The rest of MedLum continues to work normally.</p>}
    </section>}

    {canManage && <section className="rounded-2xl bg-white p-4 border mb-3"><h2 className="font-semibold">Patient Portal Access</h2><p className="text-xs text-gray-500 mt-1">Create or reset a patient's portal password. Patients only receive read-only access to their own records.</p><form onSubmit={portal} className="mt-3 space-y-2"><select required value={selected} onChange={e => setSelected(e.target.value)} className="w-full rounded-xl border px-3 py-2.5 text-sm"><option value="">Select patient</option>{patients.map(p => <option key={p.id} value={p.id}>{p.name} · {p.phone}</option>)}</select><div className="flex gap-2"><input required minLength={8} type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Portal password (8+ characters)" className="flex-1 rounded-xl border px-3 py-2.5 text-sm"/><button disabled={saving} className="rounded-xl bg-[#140a1f] px-4 py-2.5 text-sm text-white">{saving ? "Saving…" : "Enable / Reset"}</button></div></form><Link href="/portal/login" target="_blank" className="inline-block mt-3 text-xs text-[#c2183a]">Open patient portal login →</Link></section>}
    ...
    <section className="rounded-2xl bg-white border overflow-hidden"><div className="px-4 py-3 border-b font-semibold">Clinic members</div><div className="divide-y">{data?.members?.map((m: any) => <div key={m.id} className="p-4 flex items-center justify-between gap-3"><div className="min-w-0"><div className="font-medium truncate">{m.doctor.name}</div><div className="text-xs text-gray-500 truncate">{m.doctor.email}</div></div>{m.role === "Owner" ? <span className="rounded-full bg-gray-100 px-2.5 py-1 text-xs">Owner</span> : canManage ? <div className="flex gap-2"><select value={m.role} onChange={e => memberAction("PATCH", m.id, e.target.value)} className="rounded-lg border px-2 py-1.5 text-xs"><option>Admin</option><option>Consultant</option><option>Staff</option></select><button onClick={() => memberAction("DELETE", m.id)} className="text-xs text-red-600">Remove</button></div> : <span className="rounded-full bg-gray-100 px-2.5 py-1 text-xs">{m.role}</span>}</div>)}</div></section>
  </AppShell>;
}
