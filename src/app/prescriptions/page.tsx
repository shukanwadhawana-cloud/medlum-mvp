...
import { apiMe, apiGetPatients, apiGetPrescriptions, apiAddPrescription, ApiDoctor } from "@/lib/api";
...
export default function PrescriptionsPage() {
  ...
  const refresh = async () => setList((await apiGetPrescriptions()) as Rx[]);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!p) return;
    const r = await apiAddPrescription({ patientId: p.id, patientName: p.name, medicines: form.medicines, advice: form.advice });
    if (r.success) {
      await refresh();
      setShowAdd(false);
  ...
  return (
    <AppShell doctor={doctor}>
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        {list.length === 0 ? (
          <div className="p-6 text-center text-gray-500 text-sm">No prescriptions yet.</div>
        ) : (
