...
export default function PatientsPage() {
  ...
  const filtered = useMemo(() => {
    const scoped = careFilter === "ALL"
      ? patients
      : patients.filter((p) => String(p.careSetting || "OPD").toUpperCase() === careFilter);
    const q = search.trim().toLowerCase();
    if (!q) return scoped;
    return scoped.filter((p) =>
      [p.name, p.phone, p.id, p.uhid, p.registrationNo, p.medlumId, p.careSetting]
        .filter(Boolean)
    ...
    <AppShell>
      <div className="p-4 space-y-3">
        ...
        <input
          ...
          placeholder="Search name, phone, UHID, MedLum ID…"
        ...
        <div className="grid lg:grid-cols-[150px_1fr] gap-3">
          <aside className="bg-white rounded-xl border shadow-sm p-2 h-fit lg:sticky lg:top-24">
            ...
            <div className="mt-3 border-t pt-2 px-2 text-[10px] text-gray-400">
              OPD and IPD remain separate workflows while the patient record remains the shared identity.
            </div>
          </aside>
                ...
                {filtered.map((p) => (
                  <div key={p.id} className="p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 grid gap-x-4 gap-y-0.5 text-[10px] text-gray-500 sm:grid-cols-2">
                          ...
                          {String(p.careSetting || "").toUpperCase() === "IPD" && (
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {p.address && <span className="px-2 py-1 rounded-full bg-gray-50 text-gray-600 text-[10px]">Address recorded</span>}
                      {p.idNumber && <span className="px-2 py-1 rounded-full bg-gray-50 text-gray-600 text-[10px]">{p.idType || "Government ID"}</span>}
                      {p.mlcNumber && <span className="px-2 py-1 rounded-full bg-red-50 text-red-700 text-[10px] font-medium">MLC {p.mlcNumber}</span>}
                      {p.allergies && <span className="px-2 py-1 rounded-full bg-red-50 text-red-700 text-[11px]">⚠ Allergy: {p.allergies}</span>}
                      {p.latestVitals && (
                    ...
                    <div className="mt-2 flex flex-wrap justify-end gap-2">
                      <Link href={`/patients/${p.id}`} className="h-8 px-3 rounded-lg border text-[11px] font-medium inline-flex items-center">Review history</Link>
                      <Link href={`/patients/${p.id}`} className="h-8 px-3 rounded-lg bg-[#c2183a] text-white text-[11px] font-medium inline-flex items-center">New consultation</Link>
                      <Link
