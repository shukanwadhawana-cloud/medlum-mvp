...
/**
 * Clinical Chart (CPRS-inspired shell)
 * - Persistent patient + visit context header
 * - Tabs: Overview | Notes | Orders | Rx | Vitals | Discharge | Billing
 * - Aggregates existing MedLum APIs only (no new clinical schema)
 * - Server tenant isolation + role gates unchanged via /api/patients/[id]
 */
...
export default function PatientClinicalChartPage() {
  ...
  if (authLoading || loading) {
    return (
      <AppShell>
        <div className="p-6 text-sm text-gray-500">Loading clinical chart…</div>
      </AppShell>
    );
  }
    ...
    return (
      <AppShell>
        <div className="p-6 space-y-3">
          ...
          <Link href="/patients" className="text-xs text-[#c2183a] font-medium">
            Back to patients
          </Link>
        </div>
      </AppShell>
  ...
  const wardLine = [profile.wardType, profile.unitType, profile.roomNumber]
    .filter(Boolean)
    .join(" · ");
      ...
      <div className="space-y-3">
        <div className="rounded-xl border bg-white overflow-hidden shadow-sm">
          <div className="grid gap-0 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x">
            ...
            <div className="p-3">
              ...
              <p className="text-sm">
                ...
                <span
                  className={
                    p.allergies && String(p.allergies).toLowerCase() !== "none"
          ...
          <div className="flex flex-wrap gap-2 border-t px-3 py-2 bg-[#f8f6fa]">
            <Link
              href={`/patients/${id}`}
              className="h-8 px-2.5 rounded-lg border bg-white text-[11px] font-medium inline-flex items-center"
            >
              Full record
            </Link>
            <Link
              href={`/patients/${id}`}
              className="h-8 px-2.5 rounded-lg bg-[#c2183a] text-white text-[11px] font-medium inline-flex items-center"
            >
              New consult
            </Link>
            <Link
        ...
        {tab === "overview" && (
          <div className="grid gap-3 lg:grid-cols-2">
            <Section title="Problems / chief complaint">
              ...
              {(latestEncounter?.diagnosis || profile.diagnosis) && (
                <p className="text-xs text-gray-600 mt-2">
                  <b>Diagnosis:</b> {latestEncounter?.diagnosis || profile.diagnosis}
                </p>
              )}
            </Section>
            <Section title="Allergies / notes">
              ...
              {freeNotes ? (
                <p className="text-xs text-gray-600 mt-2 whitespace-pre-wrap">{freeNotes}</p>
              ) : null}
            </Section>
            <Section
              title="Active lab orders"
              action={
                <Link href="/labs" className="text-[11px] text-[#c2183a] font-medium">
                  Open queue
                </Link>
              }
            >
                  ...
                  {activeLabs.slice(0, 8).map((l: any) => (
                    <li key={l.id} className="flex justify-between gap-2 text-xs">
                      ...
                      <span className="text-gray-500 shrink-0">
                        {l.status} · {formatIst(l.orderedAt)}
                      </span>
                    </li>
                  ))}
                </ul>
                ...
                <ul className="space-y-1.5">
                  {diagnostics.slice(0, 8).map((d: any) => (
                    <li key={d.id} className="flex justify-between gap-2 text-xs">
                      ...
                      <span className="text-gray-500 shrink-0">
                        {d.status} · {formatIst(d.orderedAt || d.createdAt)}
                      </span>
                    </li>
                  ))}
                </ul>
            ...
            <Section
              ...
              action={
                <Link href="/prescriptions" className="text-[11px] text-[#c2183a] font-medium">
                  All Rx
                </Link>
              }
            >
            ...
            <Section title="Latest vitals (from encounters)">
              {!latestVitals ? (
                <Empty text="No vitals captured on encounters yet." />
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
                  ...
                  <p className="col-span-full text-[11px] text-gray-500">
                    From encounter {formatIst(latestVitals.createdAt)}
                  </p>
                </div>
              )}
            </Section>
                  ...
                  {encounters.slice(0, 6).map((e: any) => (
                    <li key={e.id} className="text-xs border-b last:border-0 pb-1.5">
                      ...
                      <p className="text-gray-600 truncate">
                        {e.chiefComplaint || e.diagnosis || "Encounter"}
                      </p>
                    </li>
                  ))}
                </ul>
        ...
        {tab === "orders" && (
          <div className="grid gap-3 lg:grid-cols-2">
                ...
                <ul className="divide-y text-xs">
                  {historyLabs.slice(0, 20).map((l: any) => (
                    <li key={l.id} className="py-2">
                      ...
                      {l.result && (
                        <p className="text-gray-600 mt-0.5 whitespace-pre-wrap">{l.result}</p>
                      )}
            ...
            <Section
              ...
              action={
                <Link href="/diagnostics" className="text-[11px] text-[#c2183a] font-medium">
                  Diagnostics
                </Link>
              }
            >
              {diagnostics.length === 0 ? (
                <Empty text="No imaging orders." />
              ) : (
                <ul className="divide-y text-xs">
                    ...
                    <li key={d.id} className="py-2 flex justify-between gap-2">
                      <div>
                        ...
                        <p className="text-gray-500">
                          {[d.modality, d.bodyPart].filter(Boolean).join(" · ")}
                        </p>
                      </div>
        ...
        {tab === "rx" && (
          ...
          >
              ...
              <ul className="space-y-3">
                {prescriptions.map((r: any) => (
                  <li key={r.id} className="rounded-lg border p-3 text-xs">
                    ...
                    <Link
                      href={`/prescriptions/print?id=${encodeURIComponent(r.id)}`}
                      className="inline-block mt-2 text-[#c2183a] font-medium"
                    >
                      Print clinical Rx
                    </Link>
                  </li>
            ...
            <p className="text-[11px] text-gray-400 mt-3">
              Inpatient scheduled MAR / dose administration is not part of this chart view.
            </p>
          </Section>
        )}

        {tab === "vitals" && (
          <Section title="Vitals from clinical encounters">
            {encounters.filter((e: any) => e.bp || e.pulse || e.temperature || e.spo2).length ===
            0 ? (
              <Empty text="No vitals recorded. Capture vitals during New consult." />
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-xs min-w-[32rem]">
                  <thead>
                    <tr className="text-left text-gray-500 border-b">
                      <th className="py-2 pr-2">When (IST)</th>
                  ...
                  <tbody>
                    {encounters
                      .filter((e: any) => e.bp || e.pulse || e.temperature || e.spo2 || e.weight)
                      .map((e: any) => (
        ...
        {tab === "discharge" && (
          <Section
            title="Discharge / IPD summary"
            action={
              <Link href="/ipd-summaries" className="text-[11px] text-[#c2183a] font-medium">
                Summaries
              </Link>
            }
          >
            <p className="text-xs text-gray-600 mb-3">
              Use IPD summaries for formal discharge documentation and clinical print. Working
              diagnosis on file:{" "}
              <b>
                {profile.workingDiagnosis ||
                  profile.diagnosis ||
            ...
            <div className="flex flex-wrap gap-2">
              <Link
                href="/ipd-summaries"
                className="h-9 px-3 rounded-lg bg-[#140a1f] text-white text-xs font-medium inline-flex items-center"
              >
                Open IPD summaries
              </Link>
              <Link
                href="/ipd/print"
                className="h-9 px-3 rounded-lg border text-xs font-medium inline-flex items-center"
              >
                IPD print
              </Link>
            </div>
        ...
        {tab === "billing" && (
          <Section
            title="Billing"
            action={
              <Link href="/billing" className="text-[11px] text-[#c2183a] font-medium">
                Billing desk
              </Link>
            }
          >
