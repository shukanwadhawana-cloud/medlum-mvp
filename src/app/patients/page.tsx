  ...
  return (
    <AppShell>
      <div className="mb-4">
        ...
        <p className="text-xs text-gray-500">Search and open the patient record</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border p-3 mb-3 sticky top-[60px] z-20">
        ...
        <input
          ...
          className="w-full h-11 px-3 rounded-lg border text-sm outline-none focus:ring-2 focus:ring-[#c2183a]/20"
        />
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        {loading ? <div className="p-6 text-center text-gray-400 text-sm">Loading patients…</div> : filtered.length === 0 ? (
          <div className="p-8 text-center text-gray-500 text-sm">{search ? "No matching patient." : "No patients yet."}</div>
        ) : (
                ...
                <div key={p.id} className="p-3">
                  <div className="flex items-start justify-between gap-3">
                    <Link href={`/patients/${p.id}`} className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">{p.age} yrs · {p.gender} · {p.phone}</p>
                    </Link>
                    <Link href={`/patients/${p.id}?quick=consult`} className="shrink-0 h-8 px-2.5 rounded-lg bg-[#c2183a] text-white text-xs font-medium flex items-center">Start Consult</Link>
                  </div>
                  ...
                  {latest ? (
                    <div className="mt-2 text-xs text-gray-500">
                      ...
                      {latest.chiefComplaint ? <span> · {latest.chiefComplaint}</span> : null}
                      {latest.followUpDate ? <span className="text-[#c2183a]"> · Follow-up {latest.followUpDate}</span> : null}
                    </div>
                  ) : <p className="mt-2 text-xs text-gray-400">No consultation recorded</p>}

                  <Link href={`/patients/${p.id}`} className="inline-block mt-2 text-xs text-[#c2183a] font-medium">Open patient →</Link>
                </div>
              );
            })}
          </div>
