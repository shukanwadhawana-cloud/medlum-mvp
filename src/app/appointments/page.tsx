...
export default function AppointmentsPage() {
  ...
  const visible = useMemo(() => {
    ...
    if (filter === "upcoming") {
      return sorted.filter((a) => a.date >= today() && a.status === "Scheduled");
    }
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      const r = await apiAddAppointment({
        ...
        consultantSpecialty: form.consultantSpecialty,
      });
      if (r.success) {
        await refresh();
        setShowAdd(false);
        ...
        setError(r.error || "Could not book appointment");
      }
    } catch {
  ...
  return (
    <AppShell>
      <div>
        ...
        {error && !showAdd && (
          <div className="mb-3 bg-red-50 text-red-700 text-sm px-3 py-2 rounded-lg">
            {error}
          </div>
        )}
        ...
        {!dataLoading && filter === "today" && (
          <div className="grid grid-cols-3 gap-2 mb-3">
            <button
              ...
              className="text-left bg-amber-50 border border-amber-100 rounded-xl p-3"
            >
              <p className="text-[11px] uppercase tracking-wide text-amber-700">Waiting</p>
              <p className="text-xl font-semibold text-amber-900">{waitingCount}</p>
            </button>
            <button
              onClick={() => setFilter("completed")}
              className="text-left bg-green-50 border border-green-100 rounded-xl p-3"
            >
          ...
          <div className="bg-white rounded-xl shadow-sm border p-3 mb-3">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <p className="text-[11px] uppercase tracking-wide text-gray-400">Next patient</p>
                <p className="font-semibold text-sm truncate">{nextPatient.patientName}</p>
                <p className="text-xs text-gray-500">
                  {nextPatient.time} · {nextPatient.type}
                  {nextPatient.consultantName ? ` · ${nextPatient.consultantName}` : ""}
                  {nextPatient.consultantSpecialty ? ` · ${nextPatient.consultantSpecialty}` : ""}
                </p>
              </div>
              <div className="flex gap-2 shrink-0">
                <Link
                  href={`/patients/${nextPatient.patientId}?appointmentId=${nextPatient.id}`}
                  className="h-9 px-3 rounded-lg bg-[#c2183a] text-white text-xs font-medium flex items-center"
                >
                  Start Consult
                </Link>
                <button
          ...
          {tabs.map(([key, label]) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs whitespace-nowrap border ${
                filter === key
                  ? "bg-[#c2183a] text-white border-[#c2183a]"
          ...
          {dataLoading ? (
            ...
            <div className="divide-y">
              {visible.map((a) => (
                <div
                  key={a.id}
                  className="px-3 py-3 flex justify-between gap-3 items-start"
                >
                  <div className="min-w-0">
                    ...
                    {a.consultantName && (
                      <p className="text-xs text-gray-700 mt-1">
                        <b>Consultant:</b> {a.consultantName}
                        {a.consultantSpecialty ? ` · ${a.consultantSpecialty}` : ""}
                      </p>
                    )}
                    <span
                  ...
                  <div className="text-right shrink-0 flex flex-col items-end gap-1">
                    {(a.status === "Scheduled" || a.status === "Waiting") && (
                      <Link
                        href={`/patients/${a.patientId}?appointmentId=${a.id}`}
                        className="text-xs text-[#c2183a] font-medium"
                      >
                        Start Consult
                      </Link>
                    )}
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 p-3">
          <div className="bg-white rounded-2xl w-full max-w-md p-4 shadow-xl">
            ...
            {error && (
              <div className="mb-2 bg-red-50 text-red-700 text-sm px-3 py-2 rounded-lg">
                {error}
              </div>
            )}
            <form onSubmit={handleAdd} className="space-y-2.5">
              ...
              >
                ...
                <option>Liver Transplant Consultation</option>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                />
                <input
                  value={form.consultantSpecialty}
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="flex-1 h-11 rounded-lg bg-[#c2183a] text-white text-sm disabled:opacity-60"
                >
                  {saving ? "Booking…" : "Book"}
                </button>
              </div>
