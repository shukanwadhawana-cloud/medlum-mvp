...
export type Patient = { id: string; doctorId: string; name: string; age: number; gender: string; phone: string; bp: string; allergies: string; notes: string; createdAt: string; };
...
const DOCTORS_KEY = "medlum_doctors_v2";
const SESSION_KEY = "medlum_session_v2";
const PATIENTS_KEY = "medlum_patients_v2";
const APPTS_KEY = "medlum_appointments_v2";
const RX_KEY = "medlum_prescriptions_v2";
const INV_KEY = "medlum_invoices_v2";
...
export function signup(data: { name: string; email: string; password: string; clinicName: string; phone: string; }) {
  ...
  if (doctors.find((d) => d.email.toLowerCase() === data.email.toLowerCase())) return { success: false as const, error: "Email already registered" };
  if (data.password.length < 8) return { success: false as const, error: "Password must be at least 8 characters" };
  const doctor: Doctor = { id: "doc_" + Date.now(), name: data.name, email: data.email.toLowerCase().trim(), password: data.password, clinicName: data.clinicName, phone: data.phone, createdAt: new Date().toISOString() };
  doctors.push(doctor); write(DOCTORS_KEY, doctors);
  try { localStorage.setItem(SESSION_KEY, JSON.stringify({ doctorId: doctor.id, email: doctor.email })); } catch {}
  return { success: true as const, doctor };
}

export function login(email: string, password: string) {
  const doctor = getDoctors().find((d) => d.email.toLowerCase() === email.toLowerCase().trim() && d.password === password);
  if (!doctor) return { success: false as const, error: "Invalid email or password" };
  try { localStorage.setItem(SESSION_KEY, JSON.stringify({ doctorId: doctor.id, email: doctor.email })); } catch {}
  return { success: true as const, doctor };
}
...
export function getCurrentDoctor(): Doctor | null {
  ...
  try {
    ...
    return getDoctors().find((d) => d.id === session.doctorId) || null;
  } catch { return null; }
}
...
export function addPatient(data: { name: string; age: number; gender: string; phone: string; bp?: string; allergies?: string; notes?: string; }) {
  const doctor = getCurrentDoctor(); if (!doctor) return { success: false as const, error: "Not authenticated" };
  const patient: Patient = { id: "pat_" + Date.now(), doctorId: doctor.id, name: data.name, age: data.age, gender: data.gender, phone: data.phone, bp: data.bp || "", allergies: data.allergies || "", notes: data.notes || "", createdAt: new Date().toISOString() };
