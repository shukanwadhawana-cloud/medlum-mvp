...
export async function GET(req: Request) {
  ...
  const [patients, encounters, appointments, invoices, payments, labOrders, diagnosticOrders, prescriptions, dispensings, claims, members, recentPatients, recentEncounters, recentAppointments] = await Promise.all([
    ...
    prisma.invoice.findMany({ where: { clinicId }, select: { total: true, amount: true, status: true, createdAt: true, doctorId: true } }),
    prisma.payment.findMany({ where: { invoice: { clinicId } }, select: { amount: true, method: true, paidAt: true, doctorId: true } }),
  ...
  const recentInvoices = invoices.filter(x => x.createdAt >= start);
  const recentPayments = payments.filter(x => x.paidAt >= start);
  ...
  const invoiceStatuses = invoices.reduce((m:Record<string,number>, x) => { m[x.status] = (m[x.status] || 0) + 1; return m; }, {});
  const appointmentStatuses = recentAppointments.reduce((m:Record<string,number>, x) => { m[x.status] = (m[x.status] || 0) + 1; return m; }, {});
  const claimStatuses = claims.reduce((m:Record<string,number>, x) => { m[x.status] = (m[x.status] || 0) + 1; return m; }, {});
  const trend = Array.from({ length: days }, (_, i) => { const d = new Date(start); d.setDate(start.getDate() + i); return { date: dayKey(d), patients: 0, encounters: 0, billed: 0, collected: 0 }; });
  const trendMap = new Map(trend.map(x => [x.date, x]));
  recentPatients.forEach(x => trendMap.get(dayKey(x.createdAt))!.patients++);
  recentEncounters.forEach(x => trendMap.get(dayKey(x.createdAt))!.encounters++);
  recentInvoices.forEach(x => trendMap.get(dayKey(x.createdAt))!.billed += Number(x.total ?? x.amount ?? 0));
  recentPayments.forEach(x => trendMap.get(dayKey(x.paidAt))!.collected += Number(x.amount || 0));
  const consultantMap = new Map<string, { doctorId:string; name:string; invoices:number; billed:number; collected:number }>();
  members.forEach(m => consultantMap.set(m.doctor.id, { doctorId:m.doctor.id, name:m.doctor.name, invoices:0, billed:0, collected:0 }));
  invoices.forEach(x => { const c=consultantMap.get(x.doctorId); if(c){c.invoices++;c.billed+=Number(x.total??x.amount??0)} });
  payments.forEach(x => { const c=consultantMap.get(x.doctorId); if(c)c.collected+=Number(x.amount||0) });
  return NextResponse.json({ periodDays:days, periodStart:start.toISOString(), counts:{patients,encounters,appointments,invoices:invoices.length,payments:payments.length,labOrders,diagnosticOrders,prescriptions,dispensings,claims:claims.length}, periodCounts:{patients:recentPatients.length,encounters:recentEncounters.length,appointments:recentAppointments.length,invoices:recentInvoices.length,payments:recentPayments.length}, revenue:{billed,collected,outstanding,periodBilled:recentBilled,periodCollected:recentCollected,collectionRate:billed>0?collected/billed:0}, insurance:{requested:claimRequested,approved:claimApproved,settled:claimSettled,byStatus:claimStatuses}, paymentMethods,invoiceStatuses,appointmentStatuses,trend,consultants:Array.from(consultantMap.values()).sort((a,b)=>b.collected-a.collected),generatedAt:new Date().toISOString() }, { headers:{"Cache-Control":"no-store"} });
}
