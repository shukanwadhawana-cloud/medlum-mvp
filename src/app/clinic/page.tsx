...
export default function ClinicPage() {
  ...
  async function createStaff(e:React.FormEvent){
    ...
    try{
      ...
      setStaffName("");setStaffEmail("");setStaffPhone("");setStaffPassword("");setMessage(j.staffCode?`Staff account created (${j.staffCode}).`:`Staff account created and added to this hospital.`);await load();
  ...
  return <AppShell>
    ...
    {canManage&&clinicActive&&<section className="rounded-2xl bg-white p-4 border mb-3"><h2 className="font-semibold">Staff Management</h2><p className="mt-1 text-xs text-gray-500">Create individual hospital logins and assign department roles. Staff IDs are allocated server-side and cannot be edited by staff.</p><form onSubmit={createStaff} className="mt-3 grid gap-2 md:grid-cols-2"><input required value={staffName} onChange={e=>setStaffName(e.target.value)} placeholder="Full name" className="w-full rounded-xl border px-3 py-2.5 text-sm"/><input required type="email" value={staffEmail} onChange={e=>setStaffEmail(e.target.value)} placeholder="Email / login" className="w-full rounded-xl border px-3 py-2.5 text-sm"/><input required value={staffPhone} onChange={e=>setStaffPhone(e.target.value)} placeholder="Phone" className="w-full rounded-xl border px-3 py-2.5 text-sm"/><input required minLength={8} type="password" value={staffPassword} onChange={e=>setStaffPassword(e.target.value)} placeholder="Initial password (8+ characters)" className="w-full rounded-xl border px-3 py-2.5 text-sm"/><select value={staffRole} onChange={e=>setStaffRole(e.target.value)} className="w-full rounded-xl border px-3 py-2.5 text-sm bg-white">{STAFF_ROLES.map(r=><option key={r}>{r}</option>)}</select><button disabled={saving} className="rounded-xl bg-[#140a1f] px-4 py-2.5 text-sm font-medium text-white disabled:opacity-50">{saving?"Creating…":"Create staff account"}</button></form><div className="mt-3 rounded-xl bg-gray-50 p-3 text-xs text-gray-600">Staff ID is permanent clinical identity (e.g. DOC-0001). Role changes do not reissue the ID. Deactivation preserves it.</div></section>}
    ...
    <section className="rounded-2xl bg-white border overflow-hidden"><div className="px-4 py-3 border-b font-semibold">Hospital staff & members</div><div className="divide-y">{data?.members?.map((m:any)=><div key={m.id} className="p-4 flex items-center justify-between gap-3"><div className="min-w-0"><div className="font-medium truncate">{m.doctor?.name}</div><div className="text-xs text-gray-500 truncate">{m.staffCode?<span className="mr-1 font-semibold text-[#140a1f]">{m.staffCode}</span>:null}{m.designation||m.role}{m.department?` · ${m.department}`:""}</div><div className="text-xs text-gray-500 truncate">{m.doctor?.email}{m.doctor?.phone?" · "+m.doctor.phone:""}</div><div className="mt-1 text-[11px] text-gray-500">{m.isActive&&m.doctor?.isActive?"Active":"Inactive"}{m.staffCode?` · ID retained`:""}</div></div>{m.role==="Owner"?<span className="rounded-full bg-gray-100 px-2.5 py-1 text-xs">Owner</span>:canManageStaff&&clinicActive?<div className="flex gap-2 items-center"><select value={m.role} onChange={e=>memberAction("role",m.id,e.target.value)} className="rounded-lg border px-2 py-1.5 text-xs">{STAFF_ROLES.map(r=><option key={r}>{r}</option>)}</select><button onClick={()=>memberAction(m.isActive?"deactivate":"reactivate",m.id)} className="text-xs text-[#c2183a]">{m.isActive?"Deactivate":"Reactivate"}</button></div>:<span className="rounded-full bg-gray-100 px-2.5 py-1 text-xs">{m.role}</span>}</div>)}</div></section>
  </AppShell>;
}
