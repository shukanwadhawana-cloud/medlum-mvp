...
const blank={dateTime:"",diagnosis:"",hpi:"",allergy:"",examTime:"",bp:"",pulse:"",rr:"",spo2:"",temperature:"",cns:"",cvs:"",rs:"",perAbdomen:"",assessment:"",workingDiagnosis:"",treatmentGiven:"",course:"",procedures:"",investigations:"",dischargeTreatment:"",followUpDate:""};
...
export default function IPDSummaries(){
 ...
 const[patients,setPatients]=useState<any[]>([]),[selected,setSelected]=useState<any>(null),[type,setType]=useState(TYPES[0]),[s,setS]=useState({...blank,dateTime:new Date().toISOString().slice(0,16)}),[msg,setMsg]=useState(""),[error,setError]=useState(""),[saving,setSaving]=useState(false),[discharging,setDischarging]=useState(false);
 ...
 const save=async(e:React.FormEvent)=>{
   ...
   try{
     ...
     setMsg(`${type} saved to IPD documentation timeline`);
     await load();
 ...
 const discharge=async()=>{
   if(!selected||type!=="Discharge Summary"||discharging)return;
   const confirmed=window.confirm(`Discharge ${selected.name} from IPD? This is the final patient discharge action.`);
   if(!confirmed)return;
   setDischarging(true);setError("");setMsg("");
   try{
     const r=await fetch("/api/patients/lifecycle",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({patientId:selected.id,action:"discharge",reason:"Discharge Summary completed"})});
     const d=await r.json().catch(()=>({}));
     if(!r.ok||!d.success)throw new Error(d.error||"Could not discharge patient");
     setMsg(`${selected.name} discharged successfully`);
     setSelected(null);
     await load();
