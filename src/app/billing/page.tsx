...
import { apiMe, apiGetPatients, apiGetInvoices, apiAddInvoice, apiUpdateInvoiceStatus, ApiDoctor } from "@/lib/api";
...
export default function BillingPage() {
  ...
  const refresh = async () => setInvoices((await apiGetInvoices()) as Invoice[]);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!p) return;
    const r = await apiAddInvoice({ patientId: p.id, patientName: p.name, amount: parseFloat(form.amount) || 0, note: form.note });
    if (r.success) {
      await refresh();
      setShowAdd(false);
      setForm({ patientId: "", amount: "", note: "" });
    }
  };
          ...
          <div className="divide-y">{invoices.map((inv) => (
            <div key={inv.id} className="px-3 py-2.5 flex justify-between gap-2 items-center">
              <div>
                ...
                <p className="text-xs text-gray-500">{inv.note || "Fee"} · {new Date(inv.createdAt).toLocaleDateString()}</p>
              </div>
              <div className="text-right shrink-0">
                <p className="font-semibold text-sm">₹{inv.amount}</p>
                <p className="text-xs">{inv.status}</p>
                {inv.status !== "Paid" && (
                  <button onClick={async () => { await apiUpdateInvoiceStatus(inv.id, "Paid"); await refresh(); }} className="text-xs text-green-600">Mark Paid</button>
                )}
              </div>
            </div>
          ))}</div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md p-4 shadow-xl">
            ...
            <form onSubmit={handleAdd} className="space-y-2.5">
              ...
              <div className="flex gap-2">
                ...
                <button type="submit" className="flex-1 h-10 rounded-lg bg-[#c2183a] text-white text-sm">Create</button>
              </div>
            </form>
          </div>
        </div>
