...
export default function PrescriptionsPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!form.medicines.trim()) { setError("Enter medicines"); return; }
    setSaving(true);
    try {
      const r = await apiAddPrescription({ patientId: p.id, patientName: p.name, medicines: form.medicines.trim(), advice: form.advice.trim() });
      if (r.success) { await refresh(); setShowAdd(false); setForm({ patientId: "", medicines: "", advice: "" }); }
  ...
  return (
    <AppShell>
      <div className="flex items-center justify-between mb-4 gap-2">
        ...
        <button type="button" onClick={() => { setError(""); setShowAdd(true); }} disabled={patients.length === 0} className="h-9 px-3 rounded-lg bg-[#c2183a] text-white text-sm font-medium disabled:opacity-40">+ Rx</button>
      </div>
        ...
        {dataLoading ? <div className="p-6 text-center text-gray-400 text-sm">Loading…</div>
          ...
          : <div className="divide-y">{list.map((r) => (
            <div key={r.id} className="px-3 py-2.5">
              <div className="flex justify-between gap-2"><p className="font-medium text-sm">{r.patientName}</p><span className="text-xs text-gray-400 shrink-0">{r.createdAt ? new Date(r.createdAt).toLocaleDateString() : ""}</span></div>
              <p className="text-xs text-gray-600 mt-1 whitespace-pre-wrap"><b>Meds:</b> {r.medicines}</p>
              {r.advice ? <p className="text-xs text-gray-500 mt-0.5 whitespace-pre-wrap"><b>Advice:</b> {r.advice}</p> : null}
            </div>
          ))}</div>}
      </div>
      {showAdd && (
        <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 p-3">
          <div className="bg-white rounded-2xl w-full max-w-md p-4 shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleAdd} className="space-y-2.5">
              ...
              <textarea required rows={4} value={form.medicines} onChange={(e) => setForm({ ...form, medicines: e.target.value })} className="w-full px-3 py-2 rounded-lg border text-sm" placeholder={"Medicines *\nTab. Paracetamol 500mg 1-0-1 x 3 days\nCap. Amoxicillin 500mg 1-0-1 x 5 days"} />
              <textarea rows={2} value={form.advice} onChange={(e) => setForm({ ...form, advice: e.target.value })} className="w-full px-3 py-2 rounded-lg border text-sm" placeholder="Advice / instructions" />
              <div className="flex gap-2">
                ...
                <button type="submit" disabled={saving} className="flex-1 h-11 rounded-lg bg-[#c2183a] text-white text-sm disabled:opacity-60">{saving ? "Saving…" : "Save Rx"}</button>
              </div>
            </form>
          </div>
        </div>
