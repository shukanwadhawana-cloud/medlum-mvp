...
export async function POST(req: Request) {
  ...
  const message = update?.message; const from = message?.from; const chat = message?.chat; const text = String(message?.text || "");
  if (!from?.id || !chat?.id || !text.startsWith("/start")) return NextResponse.json({ ok: true });
  const token = text.slice(6).trim();
  if (!token) { await sendTelegramMessage(String(chat.id), "MedLum: open Connect Telegram from your MedLum account to finish linking."); return NextResponse.json({ ok: true }); }
  ...
  await sendTelegramMessage(String(chat.id), "MedLum Telegram is now linked. Future login verification codes will be sent here.");
  return NextResponse.json({ ok: true });
}

export async function GET() {
  return NextResponse.json({ ok: true, service: "medlum-telegram-webhook" });
}
